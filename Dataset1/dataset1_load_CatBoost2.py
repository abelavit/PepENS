...
## Prepare data
Dataset_test_tsv = pd.read_table("Dataset1_test.tsv")
Dataset_train_tsv = pd.read_table("Dataset1_train.tsv")
...
for i in range(len(Dataset_test_tsv)):
    for j in range(len(Proteins)):
        if (Dataset_test_tsv['seq'][i].upper() == Proteins['Prot_seq'][j].upper()):
            if(Proteins['Prot_len'][j] > 30) & (Proteins['Prot_label'][j].count('1') >= 3):            
...
for i in range(len(Dataset_test_tsv)):
    for j in range(len(Proteins2)):
        if (Dataset_test_tsv['seq'][i].upper() == Proteins2['Prot_seq'][j].upper()):
            if(Proteins2['Prot_len'][j] > 30) & (Proteins2['Prot_label'][j].count('1') >= 3): 
...
for i in range(len(Dataset_test_tsv)):
    for j in range(len(Proteins3)):
        if (Dataset_test_tsv['seq'][i].upper() == Proteins3['Prot_seq'][j].upper()):
            if(Proteins3['Prot_len'][j] > 30) & (Proteins3['Prot_label'][j].count('1') >= 3):
...
Negative_Samples = Train_Negatives_All.sample(n=round(len(Train_Positives)*1.5), random_state=102)
...
#model details: CatBoostClassifier(random_state=0,depth=9, iterations=1100, learning_rate=0.1, eval_metric='AUC', loss_function='Logloss')
