...
#Negative_Samples.to_excel("Train_negative_set.xlsx")
...
#model = LogisticRegression(random_state=0, penalty=list(params.values())[1], tol=list(params.values())[3], C=list(params.values())[0], solver = list(params.values())[2])
file = open("dataset2_LogisticRegression_model.pkl",'rb') #{'C': 0.001, 'max_iter': 600, 'penalty': 'l2', 'solver': 'liblinear'}
model = pickle.load(file)

# do predictions of the trained model on test data
predictions = model.predict(X_independent_test)
pred_probs = model.predict_proba(X_independent_test) # for AUC calculation
