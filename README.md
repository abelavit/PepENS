...
The users can use the PepENS tool to predict the peptide binding sites in their protein(s). The script 'PepENS_user.py' can be run to achieve this. The users would be need to extract the PSSM, T5, and the HSE features prior to using the tool. Example protein files (HSE_Features_user.dat, T5_Features_user.dat, and PSSM_Features_user.dat) are provided to guide file setup corresponding to each type of feature.      
## Packages 
python 3.8.20  
numpy 1.24.3  
pandas 1.5.0   
