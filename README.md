...
<img width="2288" height="3490" alt="Architecture" src="https://github.com/user-attachments/assets/fbbc8fdf-56a1-4cc7-8151-b7bb11e1d80d" />
...
The PepENS tool allows users to predict peptide binding sites in their protein(s). To run the tool, execute the script PepENS_user.py (located in the Dataset1 directory). Before using the tool, users must generate three types of features: the PSSM (see [PSI-BLAST tutorial](https://www.ncbi.nlm.nih.gov/books/NBK2590/)), the Transformer embeddings (see [ProtTrans usage guide](https://github.com/agemagician/ProtTrans)), and the HSE features (via [hsexpo program](https://github.com/biopython/biopython/blob/master/Scripts/PDB/hsexpo.py) with the arguments .pdb as input file and CN, HSEBD, and HSEBU as the Exposure Type). Example input files—HSE_Features_user.dat, T5_Features_user.dat, and PSSM_Features_user.dat—are provided for the PepENS_user.py script to demonstrate the correct file format and setup.
